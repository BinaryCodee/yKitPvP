...
import me.sonoricky.ykitpvp.events.BountyClaimEvent;
import me.sonoricky.ykitpvp.events.KillStreakEvent;
...
public class PlayerListener implements Listener {
    ...
    public void onPotionPickup(PlayerPickupItemEvent event) {
        if (event.getItem().getItemStack().getType().name().contains("POTION")) {
            Player player = event.getPlayer();
            Bukkit.dispatchCommand(player, "stack");
        }
    }
    ...
    public void onPlayerInteract(PlayerInteractEvent event) {
        if (event.getAction() == Action.RIGHT_CLICK_BLOCK && event.getClickedBlock().getType() == Material.CAULDRON) {
            event.getPlayer().openInventory(Bukkit.getServer().createInventory(null, 54, ChatColor.GRAY + "Cestino"));
            }

        if (event.getAction() == Action.RIGHT_CLICK_AIR || event.getAction() == Action.RIGHT_CLICK_BLOCK) {
            ItemStack item = event.getPlayer().getInventory().getItemInHand();
            if (item.getType() == Material.FEATHER) {
                event.getPlayer().addPotionEffect(new PotionEffect(PotionEffectType.SPEED, 5, 2));
